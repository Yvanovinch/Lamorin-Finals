<template>
	<div class="mx-auto max-w-5xl py-5">
		...
		<div>
			...
			<div v-else>
				<table class="min-w-full text-left text-sm font-light">
					...
					<tbody>
						<tr class="border-b dark:border-neutral-500" v-for="user in users" :key="user.id">
							...
							<td class="whitespace-nowrap px-6 py-2">
								<a class="cursor-pointer hover:underline hover:text-gray-700 mr-3" @click="viewUser(user.id)">View</a>
								<a class="cursor-pointer hover:underline hover:text-gray-700 mr-3" @click="editUser(user.id)">Edit</a>
							</td>
						</tr>
					</tbody>
				</table>
		...
		<div class="flex gap-2">
			<FormButton class="mt-5" type="button" buttonStyle="primary" @click="previousPage" :disabled="currentPage === 1">
				Previous
			</FormButton>
			<FormButton class="mt-5" type="button" buttonStyle="primary" @click="nextPage"
